...
class SparseRetrieval:
    ...
    def get_sparse_embedding(self) -> NoReturn:
        ...
        if os.path.isfile(emd_path) and os.path.isfile(bm25v_path):
            ...
            self.p_embedding = self.bm25v.fit_transform(self.contexts)
            print(self.p_embedding.shape)
    ...
    def get_relevant_doc(self, query: str, k: Optional[int] = 1) -> Tuple[List, List]:
        ...
        with timer("transform"):
            query_vec = self.bm25v.transform([query])
        assert (
            np.sum(query_vec) != 0
    ...
    def get_relevant_doc_bulk(
        ...
        query_vec = self.bm25v.transform(queries)
        assert (
            np.sum(query_vec) != 0
        ), "오류가 발생했습니다. 이 오류는 보통 query에 vectorizer의 vocab에 없는 단어만 존재하는 경우 발생합니다."
    ...
    def get_relevant_doc_bulk_faiss(
        ...
        query_vecs = self.bm25v.transform(queries)
        assert (
            np.sum(query_vecs) != 0
        ), "오류가 발생했습니다. 이 오류는 보통 query에 vectorizer의 vocab에 없는 단어만 존재하는 경우 발생합니다."
